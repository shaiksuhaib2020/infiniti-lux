...
class App {
  ...
  onTouchUp(e) {
    ...
    const distance = Math.sqrt(
      Math.pow(upX - this.start, 2) + Math.pow(upY - this.startY, 2)
    );
    const isDrag = distance > 6;
    
    if (!isDrag && this.onSelect && this.items) {
      // Determine centred item
      const currentOffset = this.scroll.current;
      const itemSpacing = this.itemWidth;
      const centredIndex = Math.round(currentOffset / itemSpacing) % this.items.length;
      const normalisedIndex = ((centredIndex % this.items.length) + this.items.length) % this.items.length;
      
      this.onSelect(this.items[normalisedIndex]);
    }
